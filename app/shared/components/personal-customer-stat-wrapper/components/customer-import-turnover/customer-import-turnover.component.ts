...
@Component({
  selector: 'app-customer-import-turnover',
  templateUrl: './customer-import-turnover.component.html',
  styleUrls: ['./customer-import-turnover.component.scss']
})

export class CustomerImportTurnoverComponent implements OnInit, OnDestroy{
...
importTypes = { importBill : "IMPORT_BILL", importDc : "IMPORT_DC", importOutward: "IMPORT_OUTWARD"}
accountNumberDB = "-"
createdDate = "-"
currencyArray : any[]=[]
  constructor(
  ...
  getExportData(){
    ...
    if(requestBody.facilityPaperId == null || requestBody.cusId == null){
      ...
      this.fincacleService.getImportDataDB(requestBody).then((res)=>{
      data = res? res: [];
 ...
 async onSubmit(){
      ...
      for(let year of this.selectedYearArray){
          ...
          try{
            ...
            if(this.isFinancialYear){
             ...
             data = await this.fincacleService.getImportDataFinancial(reqestBody);
            }else{
              data = await this.fincacleService.getImportDataCalender(reqestBody);
            }
  ...
  isUserHasAccess(){
   ...
   sortDataByYear(volumeList:any): any {
