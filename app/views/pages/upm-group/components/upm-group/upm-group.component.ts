...
@Component({
  selector: 'app-upm-group',
  templateUrl: './upm-group.component.html',
  styleUrls: ['./upm-group.component.scss']
})
export class UpmGroupComponent implements OnInit, OnDestroy {
  ...
  pageDataDTOs: any = [];
  tableColumns: any = ['Group Code', 'Reference Name', 'Status', 'Approval Status', 'Approved Date', 'Approved By'];

  optionsSelect = Constants.statusOptionsSelect;
  approveStatusOptionsSelect = Constants.approveStatusOptionsSelect;

  constructor(public pageService: UpmGroupService,
  ...
  loadSelectedItem(itemID) {
    ...
    this.router.navigate(['/upm-group/add-edit']);
  }
