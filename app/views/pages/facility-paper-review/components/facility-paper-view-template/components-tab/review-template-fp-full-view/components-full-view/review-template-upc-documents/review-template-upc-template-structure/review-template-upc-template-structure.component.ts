...
import {UpcTemplateAddDataComponent} from "../../../../../../../../facility-paper/components/facility-paper-add-edit/components/fp-editor/upc-template-structure/upc-template-add-data/upc-template-add-data.component";
import {ConfirmationDialogComponent} from "../../../../../../../../../../shared/components/confirmation-dialog/confirmation-dialog.component";

@Component({
  selector: 'app-review-template-upc-template-structure',
  templateUrl: './review-template-upc-template-structure.component.html',
  styleUrls: ['./review-template-upc-template-structure.component.scss']
})
export class ReviewTemplateUpcTemplateStructureComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.privilegeService.hasPrivilege(this.masterDataPrivilege.ICAS_SETTINGS_FACILITY_PAPER_EDIT)) {
  ...
  openModalTemplateData($event, treeModel, node) {
    ...
    this.modalRef = this.mdbModalService.show(UpcTemplateAddDataComponent, {
      ...
      data: {
        heading: "comming dto",
        content: {treeModel: treeModel, node: node, dataToEdit: this.dataMap[node.data.item.upcSectionID]},
      }
    });
  ...
  setDataMap(nodes) {
    for (var item of nodes) {
      if (item.children.length == 0) {
        this.dataMap[item.item.upcSectionID] = item.item.data ? item.item.data : '';
      } else {
        this.dataMap[item.item.upcSectionID] = item.item.data ? item.item.data : '';
  ...
  isApproveStatus() {
    ...
    isApproved = this.facilityPaper.currentFacilityPaperStatus == this.facilityPaperStatusConst.APPROVED;
  ...
  isRejected() {
    ...
    isRejected = this.facilityPaper.currentFacilityPaperStatus == this.facilityPaperStatusConst.REJECTED;
